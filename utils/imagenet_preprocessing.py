...
def preprocess_image(image_buffer, output_height, output_width,
                     num_channels, is_training=False, new_ver=False):
    ...
    if not new_ver:
        if is_training:
            ...
            bbox = np.array([0, 0, 1, 1], np.float32).reshape([1, 1, 4])
        ...
        if is_training:
            ...
            # image = _resize_image(image, output_height, output_width)
            image = tf.image.decode_jpeg(image_buffer, channels=num_channels)
            # resize_side = tf.random.uniform([], minval=_RESIZE_MIN, maxval=_RESIZE_MAX + 1, dtype=tf.int32)
            # image = _aspect_preserving_resize(image, _RESIZE_MIN)
            image = tf.image.random_crop(image, size=[output_height, output_width, num_channels])
