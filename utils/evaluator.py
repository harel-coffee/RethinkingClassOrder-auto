...
def evaluate(model, class_inc, args, STAGE_FOLDER='1st_stage', use_embedding=False, memory=None):
    ...
    print('INCREMENT %d (%s): Top-1 ACC: %.2f, Top-5 ACC: %.2f, Harmonic Mean: %.2f' % (
        class_inc.group_idx + 1, STAGE_FOLDER, top1_acc * 100., top5_acc, harmonic_mean))
    ...
    if class_inc.is_final_inc and not args.base_model:
        vis_final_reordered_conf_mat(test_conf_mat_filename, class_inc.name_order, range(len(class_inc.cumul_wnids)),
                                     class_inc.dataset.NUM_MAX_TEST_SAMPLES_PER_CLASS, class_inc.cumul_wnids,
