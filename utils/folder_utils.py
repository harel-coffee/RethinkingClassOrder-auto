...
def gen_base_param_str(args):
    params_str = '%s_%s' % (args.network, args.baseline)
...
def gen_inc_param_str(args):
    ...
    # 2. rehearsal
    if args.joint_training and not args.base_model:
...
def gen_dataset_str(args):
    if args.dataset == 'imagenet':
        ...
        dataset_str += '_%s' % args.order_subset
...
def create_folder(args):
    ...
    OUTPUT_FOLDER = os.path.join('result', dataset_str,
                                 'base_%d' % args.base_cl if args.base_model else 'base_%d_inc_%d_total_%d' % (
                                     args.base_cl, args.nb_cl, args.total_cl), 'seed_%d' % args.random_seed,
                                 params_str)
